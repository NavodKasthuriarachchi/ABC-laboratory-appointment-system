...
class PatientProfile {
    ...
    public function updateProfile() {
        if (isset($_POST['submit'])) {
            ...
            $sql = "UPDATE tblpatient SET FullName=:name, MobileNumber=:mobilenumber, Email=:email WHERE ID=:pid";
    ...
    public function displayProfileForm() {
        $pid = $_SESSION['damsid'];
        $sql = "SELECT * FROM tblpatient WHERE ID=:pid";
        $query = $this->dbh->prepare($sql);
        $query->bindParam(':pid', $pid, PDO::PARAM_STR);
        $query->execute();
...
$patientProfile = new PatientProfile($dbh);
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <title>DAMS - Patient Profile</title>
                ...
                <div class="col-md-12">
                    <div class="widget">
                        ...
                        <div class="widget-body">
                            <?php $patientProfile->updateProfile(); ?>
                            <?php $patientProfile->displayProfileForm(); ?>
                        </div><!-- .widget-body -->
                    </div><!-- .widget -->
                </div><!-- END column -->
            </div><!-- .row -->
